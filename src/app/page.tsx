        ...
        {liveMatches.map((match) => (
          <div key={match.id} className="card p-6 mb-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-2xl font-bold text-red-500 animate-pulse">{match.homeScore} - {match.awayScore}</span>
              <span className="text-white font-bold text-lg">{match.awayTeam}</span>
            </div>
            <div className="text-center mt-2">
              <span className="text-red-500 text-sm font-medium">{match.minute}&apos; EN VIVO</span>
            </div>
          </div>
        ))}
        <div className="mt-8">
          <Link href="/ligas" className="text-emerald-400 hover:text-emerald-300 font-medium">Ver Ligas →</Link>
        </div>
      </div>
    </div>
  );
